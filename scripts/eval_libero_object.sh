...
CHECKPOINT="${CHECKPOINT:-params::$PROJECT_DIR/lapa_checkpoints/params_v6_object}"

# Where results.json + per-episode videos go.
OUTPUT_DIR="${OUTPUT_DIR:-$PROJECT_DIR/outputs/eval_libero_object_$(date +%Y%m%d_%H%M%S)}"

# Benchmark suites: all 4 eval suites, libero_90 excluded (train-only suite).
SUITES="${SUITES:-libero_object}" # libero_goal libero_object libero_spatial}"
...
ACTION_SCALE_FILE="${ACTION_SCALE_FILE:-$PROJECT_DIR/datasets/lapa_libero/action_bins_libero_object.csv}"
