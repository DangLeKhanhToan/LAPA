...
CHECKPOINT="${CHECKPOINT:-params::$PROJECT_DIR/lapa_checkpoints/stream_params_finetune_libero_10_openvla_style}"

# Where results.json + per-episode videos go.
OUTPUT_DIR="${OUTPUT_DIR:-$PROJECT_DIR/outputs/eval_libero_openvla_style_$(date +%Y%m%d_%H%M%S)}"
...
ACTION_SCALE_FILE="${ACTION_SCALE_FILE:-$PROJECT_DIR/datasets/lapa_libero_10_openvla/action_bins.csv}"
...
ACTION_VOCAB_SIZE="${ACTION_VOCAB_SIZE:-219}"
...
FLIP_FOR_MODEL="${FLIP_FOR_MODEL:-0}"
ROT180_FOR_MODEL="${ROT180_FOR_MODEL:-1}"
...
export OUTPUT_DIR SUITES N_EVAL_PER_TASK GPU_MODE
